use crate::api::{DISPATCHER, HostCommand};
...
use types::api::command::ApiHelper;
use std::{
    ...
    match acceptor.accept(stream).await {
        Ok(stream) => {
            ...
            loop {
                ...
                if let Some(handler) = HostCommand::from_byte(cmd) {

                    handler.handler(stream.clone(), points.clone()).await;
                }
            }
        }
        Err(e) => {

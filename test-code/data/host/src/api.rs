...
use types::api::{
    command::{ApiErrorFrame, ApiHelper, ERRORCODE, NodeCommand},
...
#[async_trait]
impl ApiHelper for HostCommand {
    type Value = Arc<RwLock<P2pPoints>>;
    ...
    async fn handler(
        ...
        match &self {
            Self::AddPeer => add_peer_handle(stream, value).await?,
            Self::Peer => peer_handle(stream, value).await?,
        };

...
pub type Peer = (PeerId, SocketAddrCodec);

// TODO: to be gossip algorithm
#[repr(transparent)]
#[derive(Debug, Clone, Encode, Decode)]
pub struct PeerPool {
    points: Arc<DashMapCodec<PeerId, SocketAddrCodec>>,
}

impl PeerPool {
    pub fn new() -> Self {
        Self {
            points: Arc::new(DashMapCodec::new()),
        }
    }
    ...
    pub fn decode(value: &[u8]) -> Option<Self> {
        ...
        DashMapCodec::<PeerId, SocketAddrCodec>::decode(&mut bytes)
            .ok()
            .map(|points| PeerPool { points: Arc::new(points) })
    }
}

impl Deref for PeerPool {
    type Target = DashMapCodec<PeerId, SocketAddrCodec>;
